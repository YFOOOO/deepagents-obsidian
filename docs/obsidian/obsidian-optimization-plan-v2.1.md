...
### 方案 4: 模型适配器 (ModelAdapter) 模式 🧩

**优先级**: 🥇 P0（与智能路由并行实施）  
**难度**: ⭐⭐ 中等  
**预期收益**: 稳定工具调用逻辑、降低幻觉率、提升多模型一致性  
**适用场景**: 多模型切换、后续引入国产/本地模型、差异化指令控制

#### 设计目标
为不同模型构建“指令注入层”，解决以下问题：
1. 工具调用遗漏（模型直接回答未使用检索）
2. 工具调用顺序错误（先回答后工具）
3. 参数幻觉（虚构时间范围 / 文件路径）
4. 响应冗长与不必要推理
...
class BaseAdapter:
    ...
    def parse_tool_calls(self, response: str) -> list:  # 预留用于自定义格式解析
        return []

class QwenAdapter(BaseAdapter):
    # 中文场景优化：明确“先工具后总结”、控制冗余解释
    ...

class DeepSeekAdapter(BaseAdapter):
    # 多步推理模型：限制单轮工具数，减少无效思考文字
    ...
...
```
工具调用规则（Qwen 专用）:
- 若问题涉及检索/最新/文件内容，请先调用相关工具，再组织回答。
- 禁止跳过本地搜索直接进行网页搜索，除非命中时效关键词（最新/推荐/2025 等）。
- 避免一次调用重复近似工具；需要补充再调用时须说明新增意图。
```

#### 集成步骤
1. 初始化助手时：根据 `model_name` 选择适配器 → 生成 `enhanced_system_prompt`
2. 在消息进入前：`final_user_message = adapter.enhance_user_message(user_message, may_need_tools)`
3. 预留 `parse_tool_calls` 钩子（未来支持 XML / JSON / 自定义标签）

#### 测试用例设计
| 用例 | 输入描述 | 断言 |
|------|----------|------|
| 指令注入 | base + tools | system prompt 增加模型特定段落 |
| Qwen 工具必要性提醒 | "请搜索并总结" | user message 被附加“先工具后回答”提示 |
| DeepSeek 限制工具数 | 长复合查询 | prompt 中包含“限制初始工具调用数量” |
| 未知模型回退 | model="my-local" | 使用 BaseAdapter，不抛异常 |
| 工具遗漏率统计 | 20 条查询 | 遗漏率 < 10% |

#### 验收指标
- Qwen/DeepSeek 在混合 30 条查询中工具遗漏率 <10%
- 无“应当工具但直接回答”案例（抽样 30 条 = 0）
- Adapter 注入内容不超过基础 prompt 长度 20%
- 多模型切换后功能保持稳定（3 模型连续调用无异常）
...
### Phase 0: 架构增强 (1-2天)
- [ ] BaseAdapter + QwenAdapter + DeepSeekAdapter 初版实现
- [ ] 系统提示词重构（插入工具说明、模型特定规则）
- [ ] 单元测试：指令注入 / user message 增强
...
### Phase 3: 精细化 (5-7天)
- [ ] 结果压缩模块接入（网页搜索工具包装）
- [ ] 压缩阈值调优（150/180/200 字对比）
- [ ] Token 节省占比统计 & 回退策略

### Phase 4: 能力扩展 (7-10天)
- [ ] 时间范围搜索工具 (表达式解析: "上周"/"昨天")
- [ ] 流式输出 (astream 接入)
- [ ] 基础 CLI/插件原型（展示实时 tokens）

### Phase 5: 语义增强 (10-14天)
- [ ] 向量索引（Chroma/Faiss）+ 相关笔记推荐
- [ ] 语义缓存雏形（相似度阈值触发）
...
### V2.1 版本发布条件
- [ ] 模型适配器 (Base/Qwen/DeepSeek) 完成并通过测试
- [ ] 智能路由器实现并通过单元测试（路径覆盖率 ≥85%）
- [ ] 缓存系统命中率 > 30%（连续 3 天）
- [ ] 网页结果平均压缩率 ≥30%
- [ ] 综合平均 Token 降低 > 40%（基于 50 条混合查询）
- [ ] 工具调用格式无错误（随机抽样 30 条）
- [ ] 原有功能回归全部通过（搜索/引用/网页子代理）
- [ ] 文档与对比报告更新至 V2.1
